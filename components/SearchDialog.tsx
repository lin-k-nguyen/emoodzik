  ...
  return (
    <div role="dialog" aria-modal="true" style={{ position: 'fixed', inset: 0, zIndex: 60, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: 16 }}>
      ...
      <div style={{ position: 'relative', marginTop: '8vh', width: '100%', maxWidth: 576, overflow: 'hidden', border: '1px solid var(--border)', background: 'var(--bg)', boxShadow: '0 30px 60px rgba(0,0,0,.35)' }}>
          ...
          {results.length > 0 ? (
            <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'flex', flexDirection: 'column', gap: 4 }}>
              {results.slice(0, 6).map(p => (
                <li key={p.slug}>
                  <Link href={`/posts/${p.slug}`} onClick={onClose} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '12px 14px', textDecoration: 'none' }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      <p style={{ margin: 0, fontSize: 12, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '.05em', color: 'var(--brand)' }}>{p.category}</p>
        ...
        {results.length > 6 && (
          <Link href={`/tim-kiem?q=${encodeURIComponent(query.trim())}`} onClick={onClose} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, borderTop: '1px solid var(--border)', padding: 16, fontSize: 14, fontWeight: 500, color: 'var(--brand)', textDecoration: 'none' }}>
            Xem tất cả kết quả
