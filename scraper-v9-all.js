...
async function scrapePage(page, slug) {
  ...
  return await page.evaluate((navIds) => {
    ...
    // KEY FIX: Walk rcv-blocks in order, for each block type get content directly
    ...
    // Build a map: rcv-block index → actual DOM element containing content
    // For each rcv-block, find the NEXT sibling element that contains the actual rendered content
    // Wix renders: <div data-hook="rcv-block1" type="paragraph"></div> followed by actual <p> or image div

    // Better approach: get all content elements in DOM order and map to rcv-block positions
    // Walk the RCE children in DOM order
    const finalBlocks = []
    let keyIdx = 0
    let videoPlayerIdx = 0
    ...
    // Walk rcv-blocks in order and extract content
    rcvBlocks.forEach(rcvBlock => {
      ...
      if (type === 'paragraph' || type === 'heading1' || type === 'heading2' || type === 'heading3') {
        // Find the rendered paragraph/heading near this rcv-block
        // Walk next siblings until we find a p or h element
        let el = rcvBlock.nextElementSibling
        let found = null
        let steps = 0
        while (el && steps < 5) {
          const tag = el.tagName?.toLowerCase()
          if (tag === 'p' || tag === 'h2' || tag === 'h3' || tag === 'h1' || tag === 'blockquote') {
            found = el
            break
          }
          // Check children
...
async function main() {
  ...
  const outputFile = path.join(process.cwd(), 'emoodzik-posts-v9-all.ndjson')
  const failedFile = path.join(process.cwd(), 'emoodzik-failed-v9.txt')
  const stream = fs.createWriteStream(outputFile)
  const failed = []
  let success = 0

  console.log(`🎵 EmoodziK Scraper v9 ALL — ${slugs.length} slugs\n`)
  ...
  console.log(`\nnpx sanity dataset import emoodzik-posts-v9-all.ndjson production --project-id 22wk7h4m --dataset production --allow-replacement-characters --replace`)
}
