...
export default function BonNayPage() {
  ...
  const handleContact = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('https://formspree.io/f/mjgqorno', {
        method: 'POST',
      ...
      <div style={{ marginTop: 72, borderTop: '1px solid var(--border)', paddingTop: 56, textAlign: 'center' }}>
        ...
        {contactSent ? (
          ...
          <form onSubmit={handleContact} style={{ margin: '28px auto 0', maxWidth: 640, display: 'flex', flexDirection: 'column', gap: 16, textAlign: 'left' }}>
            ...
            {contactError && (
              <p style={{ margin: 0, fontSize: 14, color: 'var(--accent)', textAlign: 'center' }}>Có lỗi xảy ra, thử lại nhé.</p>
            )}
            <button type="submit" disabled={contactLoading} style={{ alignSelf: 'center', height: 48, border: 'none', background: 'var(--primary)', padding: '0 28px', fontSize: 15, fontWeight: 500, color: 'var(--primary-fg)', cursor: contactLoading ? 'not-allowed' : 'pointer', fontFamily: 'inherit', opacity: contactLoading ? 0.6 : 1 }}>
              {contactLoading ? 'Đang gửi…' : 'Gửi lời nhắn'}
            </button>
          </form>
        )}
      </div>
