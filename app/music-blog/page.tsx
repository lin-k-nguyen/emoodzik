...
import { getAllPosts, getPostsByCategory, getAuthor, CATEGORIES } from '@/lib/data'
...
export default function MusicBlogPage() {
  ...
  const posts = getPostsByCategory(activeCat)
      ...
      <section style={{ padding: 0 }}>
        <div style={{ position: 'relative', width: '100%', height: 'clamp(200px,28vw,400px)', overflow: 'hidden', borderBottom: '1px solid var(--border)', background: '#000' }}>
          ...
          <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, margin: '0 auto', maxWidth: 1280, padding: '0 24px 28px', textAlign: 'right' }}>
            <h1 style={{ margin: 0, fontFamily: 'var(--font-serif)', fontSize: 'clamp(38px,8vw,76px)', fontWeight: 700, lineHeight: .95, color: '#ff2e2e', textTransform: 'uppercase', textShadow: '0 2px 16px rgba(0,0,0,.85)' }}>
              Music Blog
            </h1>
          </div>
        </div>
      ...
      <section style={{ margin: '0 auto', maxWidth: 1280, padding: '28px 24px 80px' }}>
        ...
        {totalPages > 1 && (
          <nav style={{ marginTop: 64, display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
            {currentPage > 1 && (
              <button onClick={() => setPage(p => p - 1)} style={pageBtn(false)}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="m15 18-6-6 6-6" /></svg>
              </button>
            )}
            {Array.from({ length: totalPages }, (_, i) => i + 1).map(n => (
              <button key={n} onClick={() => setPage(n)} style={pageBtn(n === currentPage)}>
                {n}
              </button>
            ))}
