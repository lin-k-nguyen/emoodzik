// EmoodziK Scraper v9 EXTRA — slugs với correct mapping + artist override
const puppeteer = require('puppeteer')
const fs = require('fs')
const path = require('path')

// slug → artist slug trong Sanity
const artistOverride = {
  'wu-tang-gza-odb': 'wu-tang-clan',
  'wu-tang-ghostface-raekwon': 'wu-tang-clan',
  'wu-tang-method-man-rza': 'wu-tang-clan',
  'the-weeknd-pt-1': 'the-weeknd',
  'the-weeknd-pt-2': 'the-weeknd',
  'michael-jackson-pt-1': 'michael-jackson',
  'michael-jackson-pt-2': 'michael-jackson',
  'nas-pt-1': 'nas',
  'nas-pt-2': 'nas',
  'the-rolling-stones-pt-1': 'the-rolling-stones',
  'the-rolling-stones-pt-2': 'the-rolling-stones',
  'tupac': '2pac',
}
...
function findArtist(slug, artists) {
  ...
  const overrideSlug = artistOverride[slug]
  if (overrideSlug) {
    const found = artists.find(a => norm(a.slug?.current || '') === norm(overrideSlug))
    if (found) return found
  }
  // Default: clean slug and match
  const cleanSlug = decodeURIComponent(slug).replace(/^_+/,'')
    .replace(/-pt[-–]?\d+$/,'').replace(/-best$/,'')
...
async function main() {
  ...
  const outputFile = path.join(process.cwd(), 'emoodzik-posts-v9-extra.ndjson')
  const stream = fs.createWriteStream(outputFile)
  let success = 0

  console.log(`🎵 EmoodziK Scraper v9 EXTRA — ${slugs.length} slugs\n`)
  ...
  for (let i = 0; i < slugs.length; i++) {
    ...
    const matchedArtist = findArtist(slug, sanityArtists)
    ...
    console.log(` ✅ "${data.title.substring(0,30)}" | ${imgCount}img ${ytCount}YT | artist:${matchedArtist?.name || 'none'} | ${data.author}`)
  ...
  stream.end()
  console.log(`\n✅ ${success} thành công`)
  console.log(`\nnpx sanity dataset import emoodzik-posts-v9-extra.ndjson production --project-id 22wk7h4m --dataset production --allow-replacement-characters --replace`)
}
